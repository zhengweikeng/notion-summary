...
二、需要输出的内容
1. 先从文章的内容里提取出标题
1. 再根据文章的内容给出简单的概要，这部份可以不用太详细。
2. 最后再根据文章的内容给出详细的总结，总结的内容要尽可能的详细，要能够覆盖文章大部份的要点。

三、严格按照给定的格式进行输出
根据之前描述的要求得到标题、概要和总结后，输出的结果必须是markdown格式，并严格遵循以下模板进行输出（注意：模板中的{标题}、{概要}、{总结}要替换成你获取到的标题、概要和总结）：
